...
public class PridePreparedTest extends AbstractPrideTest {
	...
	public void testPreparedAutoInsert() throws Exception {
		...
		assumeTrue(db.autogeneratedKeysSupported(), "Database type '" + db.getDBType() + "' does not support auto generated keys!");
		AutoCustomer c = new AutoCustomer();
		c.setId(COUNT+1);
		try(PreparedInsert pi = new PreparedInsert(new String[] {"firstName"}, c.getDescriptor())) {
			pi.execute(c);
			c.find();
	...
	public void testPreparedUpdate() throws Exception {
		...
		try (PreparedUpdate pu = new PreparedUpdate(c.getDescriptor())) {
			pu.execute(c);
			c = new Customer(2);
	...
	public void testIllegalPreparedUpdate() throws Exception {
		Customer c = new Customer();
		try(PreparedUpdate pu = new PreparedUpdate(new String[] { "unknown" }, c.getDescriptor())) {
			pu.execute(c);
		}
